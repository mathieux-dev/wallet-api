...
export class TransferService {
  ...
  async createTransfer(createTransferDto: CreateTransferDto) {
    ...
    try {
      ...
      this.logger.error(
        `Erro ao tentar iniciar a transferência: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async revertTransfer(transactionId: number) {
    ...
    try {
      ...
      this.logger.log(
        `Transação com ID ${transactionId} revertida com sucesso.`,
      );
      ...
      this.logger.error(
        `Erro durante a reversão da transação com ID ${transactionId}: ${error.message}`,
      );
      throw error;
    }
  }
